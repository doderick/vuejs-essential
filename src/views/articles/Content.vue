<template>
  <div class="col-md-9 left-col pull-right">
    ...
    <!-- 评论列表 -->
    <div class="replies panel pabel-default list-panel replies-index">
...
export default {
  ...
  created() {
    ...
    if (article) {
      ...
      // 更新 likeClass, 点赞用户列表包含当前用户时，赋值为 actice，表示已赞
  ...
  mounted() {
    ...
    if (this.auth) {
      ...
      simplemde.codemirror.on('keyup', (codemirror, event) => {
        // 使用 Ctrl+Enter 时提交评论
        if (event.crtlKey && event.keyCode === 13) {
          this.comment()
          // 存在 commentId，且按下 Esc 键时取消编辑评论
    ...
    like(e) {
      ...
      if (!this.auth) {
        ...
        if (active) {
          //　清除已赞样式
          this.likeClass = ''
          //　分发 like 事件取消赞，更新实例 likeUsers 为返回的值
